...
se utiliza una unidad de tiempo para especificar unidad de tiempo en la simulación y determinas la Duración de tiempo de 1ns de los delay y 1ps para el análisis de recolección de datos. Se especifica un valor de 16 bits los cuales se encargan de llevar el numero a mostrar en los displays como visualización dinamica.
Se utiliza un divisor de frecuencia para llevar la información y saber   que parte de la se?al encenderá el 7 segmento. La frecuencia se debe dividir en la cantidad de    displays en este caso 4 y de dividen en 4 bits por cada uno. Para este proyecto es necesario implementar un reloj debido a que es una implementación de estados algorítmicos y necesita trabajar con flancos d subida, y con cada flanco incrementara el valor de la frecuencia para determinar a que display corresponde, se incluye un reset para anular el valor de todo el funcionamiento del algoritmo y volver al valor principal 0. Cada display tiene una asignación de 4 bits y su representación numérica   va de 0-F es decir si el valor 1- 0001, 2-0010, 3-001, 0100.  Cuando un display se encuentra encendido los demás permanecen apagados.
